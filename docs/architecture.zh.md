...
- `Intercept Backend`
  - 负责数据包拦截。
  - 当前已经有 WinDivert 和实验性的 `tun + smoltcp` 两条实现。
  - WinDivert 保留数据包改写路径；`tun + smoltcp` 会在用户态接收 TCP/UDP，并把接收到的 TCP 流桥接到本地代理监听端口。
- `Shared NAT`
  - 维护 `(client_ip, client_port) -> (original_destination_ip, original_destination_port)` 映射。
  - 让 WinDivert 后端能在响应返回客户端前还原原始目标元组。
- `Local Proxy`
...
3. 拦截后端截获发往 fake-ip 的流量。
   WinDivert 会把截获到的 TCP 连接改写到本地代理监听端口；`tun + smoltcp` 会在用户态接受 TCP 流并桥接到本地代理监听端口。
...
- 透明模式当前既支持成熟的 WinDivert 后端，也支持实验性的 `tun + smoltcp` 后端。
- QUIC 当前不会被代理转发。命中 fake-ip 的 QUIC 流量会被丢弃，以强制客户端回退到 TCP/TLS。
- WinDivert 仍然使用“FakeDnsServer + Intercept Backend + Local Proxy”的数据包改写架构；`tun + smoltcp` 路径则已经在 TUN 内直接回答 DNS，并使用实验性的用户态 TCP/IP 协议栈。
